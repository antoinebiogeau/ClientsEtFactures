...
class TestClient
{
	...
	void testMontantFactureNegatif()
	{
		...
		assertEquals("Le montant d'une facture ne peut pas ętre négatif.", factureMoins2.getMessage());
		Exception factureZero = assertThrows(IllegalArgumentException.class, () -> marcel.createFacture(0));
		assertEquals("Le montant d'une facture ne peut pas ętre négatif.", factureZero.getMessage());
	}
